...
for yolo_name, yolo_model in yolo_models.items():
    for sam_name, sam_model in sam_models.items():
        ...
        for i, img_file in tqdm(enumerate(img_files)):
            ...
            save_img_with_mask_and_box(img, masks, bbox, dpi=192, save_path=f'./combo_results/{yolo_name}-{sam_name}-{img_file}-mab.png')

            save_img_no_background(img, masks, dpi=192, save_path=f'./combo_results/{yolo_name}-{sam_name}-{img_file}-noback.png')

# Save times dictionary to file
with open('./combo_results/all_times.txt', 'w') as f:
